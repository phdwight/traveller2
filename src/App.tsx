...
function App() {
  ...
  useEffect(() => {
    ...
    function animateSegmentWithFly(start: [number, number], end: [number, number], onDone: () => void) {
      let t = 0;
      const duration = 1800; // Shorter duration for more natural sync
      const startTime = performance.now();
      // Start map flyTo and marker animation at the same time
      map.flyTo({ center: end, zoom: 10, speed: 1.2, duration: 1800, essential: true });
    ...
    function showAllMarkersAndRoute() {
      ...
      setMarkers(newMarkers);
      // Ensure the route line is visible below the markers
    ...
    function animateRoute() {
      if (i < coords.length - 1) {
        ...
        if (i === 0) {
          // Wait for the map to finish flying to the first place before starting the marker animation
          map.flyTo({ center: start, zoom: 10, speed: 1.2, essential: true });
          map.once('moveend', () => {
            animateSegmentWithFly(start, end, () => {
              i++;
              if (i < coords.length - 1) {
                animateRoute();
              } else {
  ...
  // Responsive maxWidth is now handled in App.css for maintainability.
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 10, maxWidth: 'var(--app-max-width)', width: '100%' }}>
        <select
      ...
      <div className="input-bar"
        style={{
          ...
          boxShadow: '0 1px 4px #0001',
          padding: '0.5em',
          maxWidth: 'calc(var(--app-max-width) - 3px)',
          width: '100%',
      ...
      {suggestions.length > 0 && (
        <ul className="suggestions-list" style={{ background: 'var(--color-surface)', color: 'var(--color-on-surface)', borderRadius: 8, boxShadow: '0 2px 8px #0002', maxWidth: 'calc(var(--app-max-width) - 3px)', width: '100%', margin: '0 auto', padding: 0, listStyle: 'none', position: 'relative', zIndex: 5 }}>
      ...
      >
        {places.map((place, idx) => (
          <span key={idx} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span
              style={{ flex: 1, color: 'var(--color-on-surface)', cursor: 'pointer' }}
              onClick={() => {
                if (coords[idx] && mapRef.current) {
                  mapRef.current.flyTo({ center: coords[idx], zoom: 10, speed: 1.2, essential: true });
                }
              }}
              tabIndex={0}
              role="button"
              aria-label={`Focus on ${place}`}
              onKeyDown={e => {
